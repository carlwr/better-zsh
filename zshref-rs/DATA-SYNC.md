...
The Rust binaries embed the corpus (~1 MB at time of writing) as JSON via `include_bytes!`. Those JSONs are generated by a deterministic TS pipeline; they flow one way (TS produces, everything else consumes) and are **not** source code.
...
- The vendor target refreshes the local `data/` — corpus, fixture — from TS output and forces vendored mode for downstream checks.
