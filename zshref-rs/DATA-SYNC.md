...
A third generated file rides along: the resolver conformance fixture. Test input, not embedded — `cargo test` reads it from wherever the corpus JSONs come from, and it ships in the `.crate` because the tests do.
...
- `build.rs` also emits a semantic input hash (`ZSHREF_BUILD_INPUT_HASH`, from the shared `data_fingerprint`): cargo cache busting, and `zshref info` reports it as `buildInputHash` so tooldef's parity suite can tell a stale binary from a fresh one.
  ...
  - vendored mode hashes every `data/*.json`, the fixture included, so its hash differs from monorepo mode's for the same TS build; the parity suite recomputes over the monorepo inputs and runs against a monorepo-mode binary
  - `every_embedded_asset_is_fingerprinted` (`cargo test`) keeps every `include_*!` asset among the hashed inputs
- `src/corpus.rs` uses `cfg`-gated path macros so the final binary contains bytes from exactly one source; the fixture's path fn beside them follows the same selection.
- The vendor target refreshes the local `data/` — corpus, fixture, tool definitions — from TS output and forces vendored mode for downstream checks.
...
The Rust build never runs TS code. Generation happens on the producer side; the producer hands Rust ready blobs. They arrive as *source-of-the-Rust-crate* at pre-publish time via the vendor target — copying from the sibling package pre-extraction, unpacking the pinned `zsh-core` release assets (corpus JSON, resolver fixture) after — and ride inside the published `.crate` thereafter. End-user machines only ever see local files.
