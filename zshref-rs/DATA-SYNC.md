...
The Rust binaries embed the corpus + tool definitions (~1 MB at time of writing) as JSON via `include_bytes!`. Those JSONs are generated by a deterministic TS pipeline; they flow one way (TS produces, everything else consumes) and are **not** source code.
