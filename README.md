...
| `@carlwr/zsh-core-tooldef` | Framework-neutral tool definitions over `zsh-core`: pure `(DocCorpus, input) → output` impls + shared `ToolDef` metadata. Consumed by the Rust binaries. | `packages/zsh-core-tooldef/` | npm, JSR |
...
Two layers, two adapters. `zsh-core` (the knowledge layer) feeds `zsh-core-tooldef` (the single `ToolDef` surface: name, JSON-Schema input, brief + long description, pure `(corpus, input) → output` `execute`). Each adapter — CLI, MCP, both in the Rust crate — is thin transport glue over the exported tool-def JSON, baked into the binaries at build time. The VS Code extension and the web SPA consume `zsh-core` directly — the extension for its editor features, the SPA at build time for its search index.
