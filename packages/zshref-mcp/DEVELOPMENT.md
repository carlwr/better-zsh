...
### Published-state verification (`verifyREGISTRY`)

Two checks depend on external registries: `testREGISTRY:install` (= `scripts/test-install.mjs`) npm-installs the packed tarball into a throwaway temp dir, which resolves the declared `@carlwr/zsh-core` and `@carlwr/zsh-core-tooldef` dependencies **from the npm registry**; `jsrREGISTRY:check` (= `deno publish --dry-run`) resolves the JSR `imports` map in `deno.json`, which points at `jsr:@carlwr/zsh-core@...` and `jsr:@carlwr/zsh-core-tooldef@...`.

Both scripts carry the `REGISTRY` marker (see `AGENTS.md` §"Script naming axes") and are wrapped by **`verifyREGISTRY`**. They are excluded from `test:integration` and any `test:*` aggregator: ordinary tests must be runnable in a fresh clone with no published-state assumptions, so that a change that adds an upstream export cannot falsely fail the MCP's own test flow before that export has been republished. Invoke `verifyREGISTRY` explicitly (manually, or as its own CI step) after the upstream alphas are known to be out on both registries with the needed surface. The CI workflow already calls `testREGISTRY:install` and `jsrREGISTRY:check` as separate jobs; the split here mirrors and formalizes that.
