...
import { zshPathSetting } from "../settings-metadata"
...
suite("build assets", () => {
  test("zshPath setting stays machine-scoped with off semantics", () => {
    assert.strictEqual(zshPathSetting.scope, "machine")
    assert.strictEqual(zshPathSetting.default, "")
    assert.strictEqual(zshPathSetting.type, "string")
    assert.match(
      zshPathSetting.markdownDescription,
